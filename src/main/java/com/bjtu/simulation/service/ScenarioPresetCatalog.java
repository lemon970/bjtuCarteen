        ...
        return List.of(
                preset("baseline_offpeak", "平峰基线", "验证系统在正常负载下的等待、座位和窗口状态。", "基线模型",
                        config("平峰基线", 2.0, 120, 300, 1000, 6, 1, 24, 0.10, 1.18,
                                85, 45, 170, 1500, 900, 2400, false, 1.0, 0.05, 0.48, "sunny", 1.0, 20260601L),
                        expected(240, "10%-22%", "0-6 分钟", "20%-45%")),
                preset("lunch_peak_pressure", "午高峰压力测试", "验证 600 人级高峰到达、排队和座位压力。", "高峰模型",
                        config("午高峰压力测试", 2.0, 300, 250, 1000, 8, 1, 40, 0.13, 1.2,
                                90, 45, 180, 1500, 900, 2400, true, 1.0, 0.05, 0.48, "sunny", 1.0, 20260512L),
                        expected(600, "14%-26%", "4-12 分钟", "40%-70%")),
                preset("seat_shortage", "座位紧张模型", "验证座位不足时找座、打包和长等待风险。", "座位压力",
                        config("座位紧张模型", 2.0, 260, 160, 1000, 8, 1, 36, 0.13, 1.2,
                                90, 45, 180, 1800, 1100, 2700, true, 1.0, 0.05, 0.50, "sunny", 1.0, 20260602L),
                        expected(520, "18%-38%", "6-15 分钟", "65%-90%")),
                preset("takeaway_intervention", "打包窗口干预", "验证增加打包窗口后是否缓解队列且不异常抬高打包率。", "干预模型",
                        config("打包窗口干预", 2.0, 300, 250, 1000, 9, 2, 40, 0.13, 1.25,
                                90, 45, 180, 1500, 900, 2400, true, 1.0, 0.05, 0.50, "sunny", 1.0, 20260603L),
                        expected(600, "16%-32%", "3-10 分钟", "40%-70%")),
                preset("rain_emergency", "雨天应急预案", "验证雨天偏好变化、窗口压力和座位压力联动。", "应急模型",
                        config("雨天应急预案", 1.5, 320, 220, 1000, 9, 2, 45, 0.20, 1.25,
                                95, 55, 190, 1500, 900, 2400, true, 1.25, 0.10, 0.55, "rainy", 1.25, 20260604L),
                        expected(480, "22%-44%", "5-14 分钟", "45%-75%")),
                preset("group_high_concentration", "高成组浓度模型", "验证拼桌、拆组、同桌入座率在高成组比例下的表现。", "群体模型",
                        groupConfig("高成组浓度模型", 1.5, 240, 180, 800, 7, 1, 32, 0.18, 1.2,
                                90, 50, 180, 1500, 900, 2400, true, 1.0, 0.10, 0.46, "sunny", 1.0, 20260605L,
                                40, 3, 5, 0, 0.85, true, 0.65, 4),
                        expected(360, "18%-40%", "3-9 分钟", "55%-85%")));
    }
