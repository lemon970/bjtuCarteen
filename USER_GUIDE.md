...
## 队列选择模型（queue_choice_model，RFC-009，仅后端 API）

UI 已不再暴露该入口。后端仍接受 `base_config.queue_choice_model`，缺省 `STATIC_SPLIT`，行为与历史一致；如需通过 JSON payload 直接调用 PREFERENCE_AWARE 路径，参考 `API.md` 中的 `base_config.queue_choice_model` 与 `base_config.window_attractiveness` 字段说明。
