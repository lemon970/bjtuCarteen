...
- 瓶颈诊断（RFC-012）：基于 `summary` 已有字段派生，识别窗口服务能力 / 座位容量 / 打包窗口 / 到达冲击 4 类资源是否触发利用率阈值（0.85），输出 `BALANCED` 兜底或带严重度（LOW/MEDIUM/HIGH）的 evidence 表。`window_id` 显示为 0-based 内部索引，不擅自 +1。
- 等待体验代理（RFC-011A）：综合代理指数 / P/I 比 / 不确定性 / 焦虑压力 / 独食调整等待。
- 公平性（RFC-011B）：等待 GINI / 非打包窗口负载 CV / 跨角色差异（solo / group / 打包三类中位数等待 max−min）。
- 打包决策解释：展示最终概率和各压力因子的贡献。
- 参数复盘：核对归一化后的核心配置。

### 启发式分析指标

瓶颈诊断、等待体验代理与公平性三类面板均为基于 `SimulationSummary` 已有字段的**派生启发式指标**，**仅用于同模型内相对比较**（如不同配置间 A/B 对照），不解释为真实感知等待时间或绝对优劣判断。

样本不足（party-weighted 样本数 < 50）时，**等待体验代理与公平性面板不显示，这是预期行为**；瓶颈诊断不依赖样本量，任何配置都会显示（包括 BALANCED 兜底）。
